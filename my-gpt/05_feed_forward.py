...
class FeedForwardLanguageModel(nn.Module):
    """Multi-head attention followed by a position-wise feed-forward network."""
    ...
    def forward(
        ...
        # Communication across T, followed by computation across C. There are
        # deliberately no residual connections or normalization in this stage.
        x = self.sa(x)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train multi-head attention followed by a feed-forward network."
    )
...
def main() -> None:
    ...
    print("Stage 5: position-wise feed-forward network")
